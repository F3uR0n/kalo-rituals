...
def drawRitualMessage():
    ...
    if ritualMessage != "":
        if time.time() - ritualMessageStart < ritualMessageDuration:
            draw_text(400, 650, ritualMessage)
...
def resetGame():
    ...
    global gameWon, levelSelected, blink, blinkStart
...
def drawBottomRooms():
    ...
    # LEFT ROOM (Room 1D)
    drawWall(x_left, y_bottom, x_mid1, y_bottom)
    drawWall(x_left, y_bottom, x_left, y_top)
...
def showScreen():
    ...
    if canPickGun and not hasGun:
        draw_text(10, 530, "Press E to pick gun But But But you only have 1 bullet, use it wisely!")

    if ghostFrozen:
        remaining = int(ghostFreezeDuration - (time.time() - ghostFreezeStart))
        if remaining < 0:
            remaining = 0
        draw_text(10, 550, f"GHOST STUNNED: {remaining}s")

    draw_text(10, 590, f"Regenerators: {regenCount}")
    if playerInvisible:
        remaining = int(invisibleDuration - (time.time() - invisibleStartTime))
        if remaining < 0:
            remaining = 0
        draw_text(10, 570, f"INVISIBLE: {remaining}s")
